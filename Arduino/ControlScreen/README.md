...
Este arduino é responsável em gerenciar estas interações e manter as informações atualizadas na tela. A comunicação é feita via UART (TX e RX).
...
- O restante das posições são usadas para guardar os endereços dos box. Cada byte guarda a linha e coluna do box num formato de binário "0b00001111", onde os primeiros 4 bits (0000) representam a linha e os outros 4 bits (1111) representam a coluna. Nesta versão foi definido um tamanho de 20 bytes por tema, ou seja pode ser definido até 20 box para cada tema. A tela nextion está controlando a inclusão para este máximo.

Importante: Se for usado um <b>novo arduino nano</b>, a memória EEPROM interna deverá passar pela seguinte inicialização:

- As primeiras 50 posições (BAT) deverão ter o valor decimal 0
- As posições restantes deverão ter o valor decimal 255

Na pasta suporte deste projeto contêm um sketch que faz isso. Carregue-o no arduino nano que irá usar e abra a serial para interagir e executar as inicializações. Após as inicializações carrege este arduino com o código do projeto.
